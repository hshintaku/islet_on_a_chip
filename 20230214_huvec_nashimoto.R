...
# detect rat genes
ratgene<-substr(fstseq@Dimnames[[1]],1,4)=="ENSG"
...
# detect rat genes
ratgene<-substr(sndseq@Dimnames[[1]],1,4)=="ENSG"
...
colnames(sndseq)<-paste0("ENSG_",substr(colnames(sndseq),1,11),romin$index)
# Create Seurat object with rat genes
...
fst.seurat[["coculture"]]<-fstmeta[colnames(fst.seurat),]$coculture
...
snd.seurat[["coculture"]]<-sndmeta[colnames(snd.seurat),]$coculture
...
# remove NA and HUVEC
#islet <- subset(subset(islet,subset=vascularization=="NA",invert=TRUE),
#                subset=vascularization=="HUVEC",invert=TRUE)

islet <- subset(subset(subset(islet,subset=vascularization=="NA",invert=TRUE),
                subset=vascularization=="Unknown",invert=TRUE),subset=lot=="Lot1",invert=TRUE)

# remove low quality data
FeatureScatter(islet,feature1 = "nCount_RNA",feature2 = "nFeature_RNA")+scale_x_log10()+scale_y_log10()
...
DimPlot(islet,group.by = "vascularization")+DimPlot(islet,group.by = "batch")+DimPlot(islet,group.by = "coculture")
...
islet <- FindNeighbors(islet, dims = 1:5)
islet <- FindClusters(islet,resolution =1)
# let's make UMAP just with PC1 and PC2
islet<-RunUMAP(islet, dims = 1:5)
...
db<-data.frame(org.Hs.egSYMBOL2EG)
...
angiogenesis.index <-grep("response to chemokine", gse_result$Description)
